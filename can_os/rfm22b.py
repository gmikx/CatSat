...
def nirq_callback(gpio_id, val):
    ...
    debug_log("nirq callback gpio %s: %s" %
              (gpio_id, val), time.strftime('%X'))
...
class Rfm22(threading.Thread):
    ...
    def put_tx_data(self, data, block=True, timeout=None):
        ...
        for x in data:
            if (x < 0 or x > 255):
                raise Rfm22Error(
                    'List can only contain bytes with value between 0 and 255!')
    ...
    def initialise(self):
        ...
        RPIO.add_interrupt_callback(
            RFM22_NIRQ_GPIO_BCM_PIN, nirq_callback, edge='falling')
        ...
        while(True):
            ...
            if self.__state == self.__STATE_SENDING:
                ...
                if(event & self.__INTERRUPT_STATUS_1_FLAG_PACKET_TRANSMITTED):
                    ...
                    event = event & ~self.__INTERRUPT_STATUS_1_FLAG_FIFO_UNDER_OVER_FLOW
                    register_data = self.__read(
                        self.__OPERATING_FUNCTION_CONTROL_2)[1]
                    self.__write(self.__OPERATING_FUNCTION_CONTROL_2,
                                 register_data | 0x01)
                    self.__write(self.__OPERATING_FUNCTION_CONTROL_2,
                                 register_data & 0xFE)
                    self.__rx_enable()
                    self.__state = self.__STATE_RECEIVING
                    debug_log("Fifo error while sending. Receiving",
                              time.strftime('%X'))
                else:
                    u'''Clear event'''
                    event = 0
                ...
                if (event & self.__INTERRUPT_STATUS_1_FLAG_PACKET_RECEIVED):
                    ...
                    packet_length = self.__read(
                        self.__RECEIVED_PACKET_LENGTH)[1]
                    ...
                    debug_log("Packet received, Bytes:",
                              packet_length, time.strftime('%X'))
                    self.__rx_enable()
                    ...
                    if(register_data & self.__INTERRUPT_STATUS_2_FLAG_RSSI):
                        ...
                        self.__rssi = self.__read(
                            self.__RECEIVED_SIGNAL_STRENGTH_INDICATOR)
                        debug_log(
                            "Channel not free, RSSI Level: ", self.__rssi)
                        ...
                        debug_log(
                            "Tx Retrigger mechanism delay !implement me! : ", delay_time)
                    else:
                        try:
                            ...
                            u'''Clear TX Fifo'''
                            self.__write(
                                self.__OPERATING_FUNCTION_CONTROL_2, register_data | 0x02)
                            self.__write(
                                self.__OPERATING_FUNCTION_CONTROL_2, register_data & 0x00)
                            ...
                            if(length <= self.__MAXIMUM_PACKET_LENGTH):
                                ...
                                self.__write(
                                    self.__TRANSMIT_PACKET_LENGTH, length)
                                ...
                                debug_log(
                                    "Maximum length exceeded could not send")
                        except Exception as e:
                            debug_log(
                                "Event but no data in queue! Exception: ", e)
